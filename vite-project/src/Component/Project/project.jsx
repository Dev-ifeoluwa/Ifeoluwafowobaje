...
import myWorksImport from "../../assets/myProject"
import './project.css';
...
export default function Project() {
  ...
  const workProject = myWorksImport.map(myWorks => 
    <div 
      key={myWorks.projectId}
      className='project' id='project'
        >
      <div className="project-list">
        <img 
          src={myWorks.projectImage} 
          alt={myWorks.projectImage + "'s banner"} 
        />
        <div className="info-project">
          <h2>{myWorks.ProjectName}</h2>
          <p>{myWorks.projectDesc}</p><hr />
          <p>{myWorks.projectTech}</p>
        </div>
      </div>
    </div>
  )
